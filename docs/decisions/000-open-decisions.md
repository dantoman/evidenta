...
---
| **OD-45** | **Privilegiile implicite fac orice tabelă nouă scriibilă.** `0001_roles.sql` acordă INSERT/UPDATE/DELETE către `evidenta_app` pentru fiecare tabelă creată de owner — corect pentru tabelele de business, greșit pentru cele globale, unde scrierea era oprită doar de absența unei politici de INSERT. Retras explicit pentru `counterparty_registry`, `feature_flag` și `release_ring`. **De decis:** dacă privilegiile implicite ar trebui restrânse la `SELECT`, cu acordarea scrierii per tabelă — mai sigur, dar face din fiecare tabelă nouă un GRANT în plus de scris | Nimic acum; cele trei tabele sunt acoperite | Înainte de F1 |
| **OD-44** | **Extinderea gardianului la coloanele `*_key`.** `C34` cere colație explicită pe coloanele de cod; gardianul verifică sufixele `code`, `idno`, `sku`, `number`, `series`. Coloanele terminate în `_key` — `capability_key`, `module_key`, `flag_key`, `parameter_key` — sunt aceeași clasă de identificatori, dar nu sunt verificate. Extinderea listei ar semnala retroactiv tabele existente și cere migrări proprii | Nimic acum; e o lacună **înregistrată**, nu tăcută | Înainte de F1 |
