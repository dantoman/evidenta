...
def world(seed: Callable[..., None]) -> dict[str, uuid.UUID]:
    ...
    for tenant_key, user_key in (("tenant_a", "user_a"), ("tenant_b", "user_b")):
        seed(
            ...
            " VALUES (%s, %s, %s, 'owner', 'active', %s, %s, %s, %s)",
            [uuid.uuid4(), ids[tenant_key], ids[user_key], now, now, now, now],
        )
...
def firm_world(seed: Callable[..., None], world: dict[str, uuid.UUID]) -> dict[str, uuid.UUID]:
    ...
    seed(
        ...
        " VALUES (%s, %s, %s, 'owner', 'active', %s, %s, %s, %s)",
        [uuid.uuid4(), ids["firm_tenant"], ids["user_f"], now, now, now, now],
    )
    seed(
...
def grant_company(seed: Callable[..., None]) -> Callable[..., uuid.UUID]:
    ...
    def make(
        ...
        *,
        ...
        seed(
            "INSERT INTO company_access (id, tenant_id, company_id, user_id, role,"
            " granted_via, engagement_id, valid_from, granted_by_user_id,"
            " created_at, updated_at)"
            " VALUES (%s, %s, %s, %s, 'accountant', %s, %s, '2020-01-01', %s, %s, %s)",
            [access_id, tenant_id, company_id, user_id, via, engagement_id, granted_by, now, now],
        )
        return access_id
